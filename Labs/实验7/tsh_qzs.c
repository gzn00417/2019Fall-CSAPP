...
int builtin_cmd(char **argv) {
	if(!strcmp(argv[0],"quit")) { //内置命令quit,退出tsh
		exit(0);
	} else if(!strcmp(argv[0],"bg") || (!strcmp(argv[0],"fg"))) { //内置命令fg或bg，执行do_fgbg,返回1
		do_bgfg(argv);
		return 1;
	} else if(!strcmp(argv[0],"jobs")) { //内置命令jobs,调用listjobs,返回1
		listjobs(jobs);
		return 1;
	}
...
void sigchld_handler(int sig) {
	...
	while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {  //while循环 回收所有僵死进程
		...
		if (WIFEXITED(status)) {        //子进程通过调用exit或者一个返回正常终止
			sigprocmask(SIG_BLOCK, &mask, &prev);         //阻塞所有的信号
